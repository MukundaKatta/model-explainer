"""FastAPI application for model-explainer."""
...
app = FastAPI(title="model-explainer", version="0.1.0", description="model-explainer API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "model-explainer", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "model-explainer", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "model-explainer", "version": "0.1.0", "ready": True,
